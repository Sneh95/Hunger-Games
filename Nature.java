...
public class Nature implements Drawable {
    ...
    private Class[] animals = {Panda.class, Boar.class,
        Pig.class, Rabbit.class};
    private Class[] edibles = {Boar.class, Pig.class};
    private Class[] endangered = {Panda.class, Rabbit.class};
    ...
    /**
     * This method draws the current Animals
     * and the current Homes of the Animals
     * @param g the component object on which to draw
     */
    public void draw(Graphics g) {
        for (Animal currentAnimals : existingAnimals) {
            currentAnimals.draw(g);
        }

        for (Home currentHomes : homes) {
            currentHomes.draw(g);
        }
    }
